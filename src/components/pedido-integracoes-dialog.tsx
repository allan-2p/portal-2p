...
import { CheckCircle2, Cloud, History, Loader2, Plug, RefreshCw, XCircle } from "lucide-react";
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Plug className="h-4 w-4" /> Integrações do pedido {d?.numero ? `#${d.numero}` : ""}
          </DialogTitle>
          <DialogDescription>
            {d?.cliente_nome ?? "Situação do envio ao SAP e ao Salesforce, com reenvio e histórico."}
          </DialogDescription>
        </DialogHeader>
        ...
        {d && (
          <div className="space-y-4">
            ...
            <section className="space-y-2 rounded-xl border border-border bg-surface/40 p-3">
              <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-primary">
                <History className="h-3.5 w-3.5" /> Histórico de integrações deste pedido
              </div>
              {logs.isLoading && (
