...
- Lista de propostas (Solar e Carregadores) mais compacta, sem rolagem lateral: proposta e cliente em duas linhas, nova coluna **Compra** (data de fechamento) e a data de criação renomeada para **Criação** como último campo. O número da **NF** passa a aparecer logo abaixo do Nº SAP (lista e cartões mobile).
