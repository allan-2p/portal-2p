...
export const INTEGRATIONS: IntegrationDef[] = [
  ...
  },
  {
    ...
    category: "Pagamentos",
    description:
      "Emissão e baixa de cobranças Pix e boleto. As APIs exigem mTLS: use o proxy mTLS externo (ITAU_PROXY_URL/ITAU_PROXY_SECRET), que guarda o certificado e resolve o token OAuth.",
    credentials: [
      { env: "ITAU_PROXY_URL", label: "URL do proxy mTLS", required: true, secret: false },
      { env: "ITAU_PROXY_SECRET", label: "Segredo do proxy mTLS", required: true, secret: true },
      ...
      { env: "ITAU_API_BASE", label: "URL base da API (modo direto)", secret: false },
      { env: "ITAU_MTLS_CERT_PEM", label: "Certificado mTLS (modo direto)", secret: true },
      { env: "ITAU_MTLS_KEY_PEM", label: "Chave privada mTLS (modo direto)", secret: true },
      { env: "ITAU_PIX_CLIENT_ID", label: "Client ID Pix (modo direto)", secret: true },
      { env: "ITAU_PIX_CLIENT_SECRET", label: "Client Secret Pix (modo direto)", secret: true },
      { env: "ITAU_BOLETO_CLIENT_ID", label: "Client ID Boleto (modo direto)", secret: true },
      { env: "ITAU_BOLETO_CLIENT_SECRET", label: "Client Secret Boleto (modo direto)", secret: true },
    ],
    note: "O modo direto (certificado no próprio portal) não funciona no runtime atual — a chamada sai sem certificado e o Itaú responde 403 C100. Enquanto o proxy não estiver configurado, a emissão fica indisponível.",
  },
