...
/**
 * Define o NCM de um ou vários produtos de uma vez. A RFC `listar_material`
 * do SAP não expõe o campo STEUC/NCM, então o vínculo é feito aqui e grava
 * também o código para exibição e para o cálculo de impostos.
 */
export const setSapProdutosNcm = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) =>
    z
      .object({
        ids: z.array(z.string().uuid()).min(1),
        ncmId: z.string().uuid().nullable(),
      })
      .parse(d),
  )
  .handler(async ({ data, context }) => {
    ...
    let ncmCodigo: string | null = null;
    if (data.ncmId) {
      const { data: ncm, error } = await context.supabase
        .from("cpo_ncm")
        .select("codigo")
        .eq("id", data.ncmId)
        .maybeSingle();
      if (error) throw new Error(error.message);
      if (!ncm) throw new Error("NCM não encontrado.");
      ncmCodigo = String(ncm.codigo).replace(/\D/g, "").slice(0, 8) || null;
    }

    const { error: updError } = await context.supabase
      .from("sap_produtos")
      .update({ ncm_id: data.ncmId, ncm_codigo: ncmCodigo })
      .in("id", data.ids);
    ...
    await recordModeration(context, {
      area: "produtos",
      action: "atualizou",
      target: `${data.ids.length} produto(s)`,
      summary: data.ncmId
        ? `NCM ${ncmCodigo ?? ""} aplicado a ${data.ids.length} produto(s)`
        : `NCM removido de ${data.ids.length} produto(s)`,
      details: { ids: data.ids, ncm_id: data.ncmId, ncm_codigo: ncmCodigo },
    });

    return { ok: true, atualizados: data.ids.length, ncm_codigo: ncmCodigo };
  });

/**
 * Preenche `ncm_codigo` nos produtos que já têm NCM vinculado mas ficaram sem
 * o código gravado (ex.: vínculos criados antes desta coluna existir).
 */
export const backfillSapProdutosNcm = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }): Promise<{ atualizados: number }> => {
    await requireAnyFeature(context, [
      { instance: "solar", feature: "admin.objetos.produtos", action: "moderar" },
      { instance: "carregadores", feature: "admin.objetos.produtos", action: "moderar" },
      { instance: "carregadores", feature: "cpo.produtos", action: "moderar" },
    ...
    const { data: produtos, error } = await context.supabase
      .from("sap_produtos")
      .select("id, ncm_id, ncm_codigo")
      .not("ncm_id", "is", null)
      .is("ncm_codigo", null);
    if (error) throw new Error(error.message);
    if (!produtos || produtos.length === 0) return { atualizados: 0 };

    const { data: ncms } = await context.supabase.from("cpo_ncm").select("id, codigo");
    const mapa = new Map((ncms ?? []).map((n: any) => [n.id as string, String(n.codigo).replace(/\D/g, "").slice(0, 8)]));

    let atualizados = 0;
    for (const p of produtos as { id: string; ncm_id: string }[]) {
      const codigo = mapa.get(p.ncm_id);
      if (!codigo) continue;
      const { error: e } = await context.supabase
        .from("sap_produtos")
        .update({ ncm_codigo: codigo })
        .eq("id", p.id);
