...
import { Instagram, Youtube, Users, Radio, Facebook, Linkedin, AlertCircle, TrendingUp, TrendingDown, Heart, MessageCircle, Eye, Sparkles } from "lucide-react";
...
import { getMetricoolBrandData, type MetricoolPost } from "@/lib/metricool.functions";
    ...
    <AppLayout>
      <div className="max-w-[1500px] mx-auto space-y-5">
        <div className="flex items-end justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Metricool + Salesforce · {range.label}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
          <KPI label="Posts no período" value={fmt(insights.total)} loading={mcQ.isLoading} />
          <KPI label="Alcance total" value={fmt(insights.totalReach)} loading={mcQ.isLoading} />
          <KPI label="Interações" value={fmt(insights.totalInteractions)} loading={mcQ.isLoading} />
          <KPI label="Eng. médio" value={`${insights.avgEngagement.toFixed(2)}%`} loading={mcQ.isLoading} />
          <KPI label="Média alcance / post" value={fmt(Math.round(insights.avgReach))} loading={mcQ.isLoading} />
        </div>
