...
export function FreteRegrasEditor({ unidade }: { unidade: UnidadeFrete }) {
  const solar = unidade === "solar";
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-border bg-card p-6 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Valem para as cotações de {solar ? "2P Solar" : "2P Carregadores"}.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="space-y-1.5">
            ...
            />
            <p className="text-[11px] text-muted-foreground">
              Acima disso, a Braspress sai da lista de opções (ex.: carregador de 80 kW).
            </p>
            <div className="flex items-center gap-2 pt-1">
              <Switch
                id="kw-ativo"
                checked={cfg.bloqueioCarregadorBraspress}
                onCheckedChange={(v) => setCfg((c) => ({ ...c, bloqueioCarregadorBraspress: v }))}
              />
              <Label htmlFor="kw-ativo" className="text-xs font-normal text-muted-foreground">
                {cfg.bloqueioCarregadorBraspress ? "Bloqueio ativo" : "Bloqueio desativado"}
              </Label>
            </div>
          </div>
          )}
      ...
      {!solar
        ? REGRAS_CARREGADORES.map((r) => (
            <section key={r.cnpj} className="rounded-2xl border border-border bg-card p-6 space-y-3">
              <div className="flex items-start gap-3">
              ...
              <ul className="border-t border-border pt-3 space-y-1">
                ...
                <li className="text-xs text-muted-foreground flex gap-2">
                  <span className="text-primary">•</span>
                  <span>
                    Regras de trilho não se aplicam aos carregadores — elas ficam em Regras de Fretes da 2P
                    Solar.
                  </span>
                </li>
              </ul>
