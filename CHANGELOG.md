...
## [Não publicado]
- Fotos de produto: carga única do acervo histórico para o bucket privado `produtos` (79 produtos sem foto verificados, 23 importados, 56 sem arquivo, 0 erros), gravando `sap_produtos.imagem_path` e registrando o relatório no log de integrações (`produtos` / `migracao-imagens`). Fotos enviadas manualmente foram preservadas. A rotina foi temporária e não ficou no código: a gestão de fotos segue exclusivamente pela tela "Catálogo de fotos".
